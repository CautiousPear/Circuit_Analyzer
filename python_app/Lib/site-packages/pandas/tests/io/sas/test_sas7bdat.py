...
def test_date_time(datapath):
    ...
    df0["DateTime"] = df0["DateTime"].astype("M8[us]")
    df0["Taiw"] = df0["Taiw"].astype("M8[s]")

    res = df0["DateTimeHi"].astype("M8[us]").dt.round("us")
    df0["DateTimeHi"] = res.astype("M8[us]")

    if not IS64:
        # No good reason for this, just what we get on the CI
        df0.loc[0, "DateTimeHi"] += np.timedelta64(1, "us")
        df0.loc[[2, 3], "DateTimeHi"] -= np.timedelta64(1, "us")
    tm.assert_frame_equal(df, df0)
...
def test_max_sas_date(datapath):
    ...
    if not IS64:
        # No good reason for this, just what we get on the CI
        expected.loc[:, "dt_as_dt"] -= np.timedelta64(1, "us")
...
def test_max_sas_date_iterator(datapath):
        ...
        pd.DataFrame(
            {
                ...
                "dt_as_dt": np.array(
                    [datetime(9999, 12, 29, 23, 59, 59, 999000)], dtype="M8[us]"
                ),
        ...
        pd.DataFrame(
            {
                ...
                "dt_as_dt": np.array(["2019-08-01 23:59:59.999"], dtype="M8[us]"),
    ...
    if not IS64:
        # No good reason for this, just what we get on the CI
        expected[0].loc[0, "dt_as_dt"] -= np.timedelta64(1, "us")
        expected[1].loc[0, "dt_as_dt"] -= np.timedelta64(1, "us")
...
def test_null_date(datapath):
    ...
    if not IS64:
        # No good reason for this, just what we get on the CI
        expected.loc[0, "datetimecol"] -= np.timedelta64(1, "us")
    tm.assert_frame_equal(df, expected)
