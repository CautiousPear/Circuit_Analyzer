...
class TestDatetimeTZDtype(Base):
    ...
    def test_construction_non_nanosecond(self):
        res = DatetimeTZDtype("us", "US/Eastern")
        assert res.unit == "us"
        assert res._creso == NpyDatetimeUnit.NPY_FR_ms.value
        assert res.str == "|M8[us]"
        assert str(res) == "datetime64[us, US/Eastern]"
        assert res.base == np.dtype("M8[us]")

    def test_day_not_supported(self):
        msg = "DatetimeTZDtype only supports s, us, us, ns units"
        with pytest.raises(ValueError, match=msg):
