...
class TestSeriesConstructors:
    ...
    def test_constructor_dtype_datetime64_10(self):
        ...
        # GH3414 related
        expected = Series(pydates, dtype="datetime64[us]")

        result = Series(Series(dates).astype(np.int64) / 1000000, dtype="M8[us]")
        tm.assert_series_equal(result, expected)

        result = Series(dates, dtype="datetime64[us]")
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize("unit", ["ns", "us", "us", "s", "h", "m", "D"])
    ...
    def test_constructor_datetime64_bigendian(self):
        # GH#30976
        ms = np.datetime64(1, "us")
        arr = np.array([np.datetime64(1, "us")], dtype=">M8[us]")

        result = Series(arr)
        expected = Series([Timestamp(ms)]).astype("M8[us]")
        assert expected.dtype == "M8[us]"
        tm.assert_series_equal(result, expected)
