...
class TestNonNano:
    ...
    def test_sub_datetime64_reso_mismatch(self):
        ...
        right = left.as_unit("us")

        result = left - right
        exp_values = np.array([0, 0, 0], dtype="m8[us]")
...
class TestDatetimeArray:
    def test_astype_ns_to_ms_near_bounds(self):
        ...
        target = ts.as_unit("us")

        dta = DatetimeArray._from_sequence([ts], dtype="M8[ns]")
        assert (dta.view("i8") == ts.as_unit("ns").value).all()

        result = dta.astype("M8[us]")
        assert result[0] == target

        expected = DatetimeArray._from_sequence([ts], dtype="M8[us]")
