...
class TestDateTime:
    ...
    def test_datetime_scalar_construction(self):
        ...
        assert_equal(np.datetime64(datetime.date(2045, 3, 25), 'us'),
    ...
    def test_timedelta_scalar_construction(self):
        ...
        assert_equal(np.timedelta64(10234760000, 'us'),
                     np.timedelta64(datetime.timedelta(
                                            microseconds=10234760000)))
        assert_equal(np.timedelta64(1023476, 'us'),
    ...
    def test_datetime_nat_casting(self):
        ...
        assert_equal(a.astype('M8[us]'), np.array('NaT', dtype='M8[us]'))
        ...
        assert_equal(np.datetime64(b, '[us]'), np.datetime64('NaT', '[us]'))
        ...
        assert_equal(np.datetime64(a, '[us]'), np.datetime64('NaT', '[us]'))
    ...
    def test_dtype_comparison(self):
        assert_(not (np.dtype('M8[us]') == np.dtype('M8[us]')))
        assert_(np.dtype('M8[us]') != np.dtype('M8[us]'))
    ...
    def test_different_unit_comparison(self):
        ...
        for unit1 in ['6h', 'h', 'm', 's', '10ms', 'us', 'us']:
            dt1 = np.dtype(f'M8[{unit1}]')
            for unit2 in ['h', 'm', 's', 'us', 'us']:
                dt2 = np.dtype(f'M8[{unit2}]')
        ...
        for unit1 in ['D', '12h', 'h', 'm', 's', '4s', 'us', 'us']:
            dt1 = np.dtype(f'M8[{unit1}]')
            for unit2 in ['D', 'h', 'm', 's', 'us', 'us']:
                dt2 = np.dtype(f'M8[{unit2}]')
    ...
    def test_datetime_no_subtract_reducelike(self):
        ...
        arr = np.array(["2021-12-02", "2019-05-12"], dtype="M8[us]")
    ...
    def test_isnat(self):
        assert_(np.isnat(np.datetime64('NaT', 'us')))
        assert_(np.isnat(np.datetime64('NaT', 'ns')))
        assert_(not np.isnat(np.datetime64('2038-01-19T03:14:07')))

        assert_(np.isnat(np.timedelta64('NaT', "us")))
        assert_(not np.isnat(np.timedelta64(34, "us")))
    ...
    def test_isfinite_scalar(self):
        ...
        assert_(not np.isfinite(np.timedelta64('NaT', "us")))
        assert_(np.isfinite(np.timedelta64(34, "us")))

    @pytest.mark.parametrize('unit', ['Y', 'M', 'W', 'D', 'h', 'm', 's', 'us',
                                      'us', 'ns', 'ps', 'fs', 'as'])
    ...
    @pytest.mark.parametrize("time_unit", [
        "Y", "M", "W", "D", "h", "m", "s", "us", "us", "ns", "ps", "fs", "as",
        # compound units
        "10D", "2M",
    ])
    ...
    @pytest.mark.parametrize('unit', ('Y', 'M', 'W', 'D', 'h', 'm', 's', 'us', 'us'))
    ...
    @pytest.mark.parametrize('unit', ('h', 'm', 's', 'us', 'us'))
    ...
    @pytest.mark.parametrize('unit', ('m', 's', 'us', 'us', 'ns', 'ps', 'fs'))
    ...
    @pytest.mark.parametrize('unit', ('W', 'D', 'h', 'm', 's', 'us', 'us'))
    ...
    @pytest.mark.parametrize('unit', ('us', 'us', 'ns', 'ps', 'fs', 'as'))
    def test_timedelta_hash_ms(self, unit):
        td = np.timedelta64(3, 'us')
        td2 = np.timedelta64(td, unit)
        _assert_equal_hash(td, td2)
