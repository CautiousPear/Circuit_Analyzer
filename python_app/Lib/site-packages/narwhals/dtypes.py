...
class Datetime(TemporalType, metaclass=_DatetimeMeta):
    ...
    Examples:
        ...
        ... )
        >>> nw.from_native(s_native, series_only=True).dtype
        Datetime(time_unit='us', time_zone='Africa/Accra')
    """
    ...
    def __init__(
        ...
        if time_unit not in {"s", "us", "us", "ns"}:
            msg = (
                "invalid `time_unit`"
                f"\n\nExpected one of {{'ns','us','us', 's'}}, got {time_unit!r}."
            )
...
class Duration(TemporalType, metaclass=_DurationMeta):
    ...
    Examples:
        ...
        ...     [[timedelta(seconds=d) for d in range(1, 4)]], type=pa.duration("us")
        ... )
        >>> nw.from_native(s_native, series_only=True).dtype
